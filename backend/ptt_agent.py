...
async def run_agent(agent: PTTAgent):
    """
    Main function to run PTT Agent with proper error handling
    
    Args:
        agent: PTTAgent instance with io handler
    """
    try:
        print("🔧 Setting up PTT Agent...")
        await agent.setup()
        
        print("🌳 Initializing task tree...")
        await agent.initialize_tree()
        
        print("🔄 Starting reasoning loop...")
        await agent.run_reasoning_loop()
        
        print("✅ PTT Agent completed successfully")
        await agent.io.output("status", "Agent execution completed successfully")
        
    except KeyboardInterrupt:
        print("⚠️ Agent interrupted by user")
        await agent.io.output("status", "Agent stopped by user")
        
    except Exception as e:
        print(f"❌ PTT Agent Error: {e}")
        import traceback
        traceback.print_exc()
        
        await agent.io.output("error", f"Fatal error: {str(e)}")
        
    finally:
        print("🧹 Cleaning up PTT Agent resources...")
        try:
            await agent.close()
        except Exception as e:
